...
import com.kanyuServer.service.impl.LoginServiceImpl;
...
//    }
    ...
    public void onMessage(String message, Session session, @PathParam("userId") String userId) {
        ...
        JSONObject obj = JSONUtil.parseObj(message);
        String receiveUser = obj.getStr("receiveUserId"); // receiveUser表示发送给哪个用户，比如 admin
        String text = obj.getStr("content"); // 发送的消息文本  hello
//        String sendUser = obj.getStr("sendUserId"); // sendUser表示谁发送的消息，比如 admin

        // {"to": "admin", "text": "聊天文本"}
        Session toSession = sessionMap.get(receiveUser); // 根据 to用户名来获取 session，再通过session发送消息文本
        if (toSession != null) {
            // 服务器端 再把消息组装一下，组装后的消息包含发送人和发送的文本内容
            // {"from": "zhang", "text": "hello"}
            JSONObject jsonObject = new JSONObject();
            jsonObject.set("sendUserId", userId);  // from 是 zhang
            jsonObject.set("receiveUserId", receiveUser);  // from 是 zhang
